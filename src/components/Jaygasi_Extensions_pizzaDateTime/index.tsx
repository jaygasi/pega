...
// interface for props
interface JaygasiExtensionsPizzaDateTimeProps extends PConnFieldProps {
...
function JaygasiExtensionsPizzaDateTime (props: JaygasiExtensionsPizzaDateTimeProps)  {
  const {
    getPConnect,
    value,
    validatemessage,
  ...
  if (displayMode === 'LABELS_LEFT' || displayMode === 'STACKED_LARGE_VAL' || displayMode === 'DISPLAY_ONLY') {
    ...
    switch (displayMode) {
      case 'DISPLAY_ONLY': {
        return ( <StyledJaygasiExtensionsPizzaDateTimeWrapper> {displayComp} </StyledJaygasiExtensionsPizzaDateTimeWrapper>);
      }
      case 'LABELS_LEFT': {
        return (
