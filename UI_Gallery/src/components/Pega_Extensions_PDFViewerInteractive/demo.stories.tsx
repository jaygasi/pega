import type { StoryObj } from '@storybook/react-webpack5';
// Import via the JS wrapper to avoid editor JSX resolution diagnostics
// Import via TypeScript wrapper to avoid editor JSX resolution diagnostics
import PegaExtensionsPDFViewerInteractive from './index.stories-wrapper';

const meta = {
  title: 'Pega Extensions/PDFViewerInteractive',
  component: PegaExtensionsPDFViewerInteractive,
  parameters: {
    docs: {
      description: {
        component: 'A component for viewing PDF documents with proper scaling and toolbar options.',
      },
    },
  },
  argTypes: {
    value: {
      control: 'text',
      description: 'PDF source URL or base64 data',
    },
    height: {
      control: 'text',
      description: 'Height of the viewer',
    },
    showToolbar: {
      control: 'boolean',
      description: 'Show PDF toolbar',
    },
    showDiagnostics: {
      control: 'boolean',
      description: 'Show diagnostic information',
    },
  },
};

export default meta;
type Story = StoryObj<typeof PegaExtensionsPDFViewerInteractive>;

export const Default: Story = {
  args: {
    value: 'data:application/pdf;base64,',
    height: '600px',
    showToolbar: true,
    showDiagnostics: false,
  },
};

export const WithDiagnostics: Story = {
  args: {
    value: 'data:application/pdf;base64,',
    height: '600px',
    showToolbar: true,
    showDiagnostics: true,
  },
};

export const NoToolbar: Story = {
  args: {
    value: 'data:application/pdf;base64,',
    height: '600px',
    showToolbar: false,
    showDiagnostics: false,
  },
};