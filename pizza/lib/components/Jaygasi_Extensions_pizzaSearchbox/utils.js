...
export function usePaginate() {
    ...
    const getInitialPaginatedRecords = (totalRecords, matchPosition) => {
    ...
    const positionMatches = (text, searchText) => {
    ...
    const fetchData = (searchText, onScroll, columns = []) => {
        let progressivePaginatedRecords;
        paginatationProperties.current.startIndex += 50;
        paginatationProperties.current.endIndex += 50;
        if (searchText === '' || searchText === undefined || searchText === null) {
            // paginate on entire resultset
            progressivePaginatedRecords = paginatationProperties.current.totalRecords.slice(paginatationProperties.current.startIndex, paginatationProperties.current.endIndex);
        }
        else if (onScroll) {
            // paginate on filtered resultset
            // @ts-ignore
            progressivePaginatedRecords = paginatationProperties.current.filteredRecords.slice(paginatationProperties.current.startIndex, paginatationProperties.current.endIndex);
        }
        else {
            // @ts-ignore
            const searchInSecondaryItem = columns.some(e => e.secondary && e.useForSearch);
            // Filter the items and paginate on filtered resultset
            const filteredRecords = paginatationProperties.current.totalRecords.filter(filteritem => 
            // @ts-ignore
            ...
            progressivePaginatedRecords = filteredRecords.slice(paginatationProperties.current.startIndex, paginatationProperties.current.endIndex);
            // @ts-ignore
    ...
    const hasMoreResults = (searchText) => {
        if (searchText === '' || searchText === undefined || searchText === null) {
            return (paginatationProperties.current.startIndex + 50 <
...
const transformSearchedItem = (displayResponseData, displayFieldMeta, columnsWithSetProperty, dataApiObj) => {
    // @ts-ignore
...
// @ts-ignore
export const updatePropertiesForNewRecord = (dataConfig, contextName, dataApiObj, setDataApiObj, displayFieldMeta, c11nEnv, columns, actions, newCaseId) => {
    PCore.getDataApi()
        ...
        dataObj
            ...
            const searchedItem = transformSearchedItem(displayResponseData, displayFieldMeta, columnsWithSetProperty, dataApiObj);
            setValuesToPropertyList(searchedItem[0], c11nEnv.getStateProps().value, searchedItem[0], columns, actions);
        })
            .finally(() => {
            // RESET matchPosition
            // @ts-ignore
