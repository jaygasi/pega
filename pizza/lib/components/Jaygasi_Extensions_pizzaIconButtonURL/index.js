...
import StyledJaygasiExtensionsPizzaIconButtonUrlWrapper from './styles';
export const textFormatter = (value) => {
    const displayComponent = (_jsx(URLDisplay, { target: '_blank', value: value, displayText: value, variant: 'link' }));
...
function JaygasiExtensionsPizzaIconButtonUrl(props) {
    ...
    function validateUrl(href) {
        return /^(?:(?:(?:https?|ftp):)?\/\/)(?:\S+(?::\S*)?@)?(?:(?!(?:10|127)(?:\.\d{1,3}){3})(?!(?:169\.254|192\.168)(?:\.\d{1,3}){2})(?!172\.(?:1[6-9]|2\d|3[0-1])(?:\.\d{1,3}){2})(?:[1-9]\d?|1\d\d|2[01]\d|22[0-3])(?:\.(?:1?\d{1,2}|2[0-4]\d|25[0-5])){2}(?:\.(?:[1-9]\d?|1\d\d|2[0-4]\d|25[0-4]))|(?:(?:[a-z\u00a1-\uffff0-9]-*)*[a-z\u00a1-\uffff0-9]+)(?:\.(?:[a-z\u00a1-\uffff0-9]-*)*[a-z\u00a1-\uffff0-9]+)*(?:\.(?:[a-z\u00a1-\uffff]{2,})))(?::\d{2,5})?(?:[/?#]\S*)?$/i.test(href);
    }
        ...
        if (showTable) {
            PCore.getDataPageUtils()
                ...
                if (response.data !== null) {
                    setStatelist(response.data.map((entry, index) => {
    ...
    if (displayMode === 'DISPLAY_ONLY') {
        ...
        return (_jsxs(StyledJaygasiExtensionsPizzaIconButtonUrlWrapper, { children: [" ", displayComp, " "] }));
    }
    return (_jsxs(StyledJaygasiExtensionsPizzaIconButtonUrlWrapper, { children: [_jsxs("div", { style: { display: 'flex' }, "data-testid": testId, children: [_jsx(Icon, { style: { height: '1.5rem', width: '1.5rem' }, 
                        // icon name is kabab case
                        name: 'arrow-right', role: 'img', "aria-label": 'arrow right' }), _jsxs(Label, { style: { margin: '0.1rem' }, children: [' ', PConnect.getLocalizedValue('State list for', '', ''), ' ', PConnect.getLocalizedValue(`${countryCode}`, '', ''), ' '] }), _jsx(Button, { icon: true, label: PConnect.getLocalizedValue('Click to show/hide state list', '', ''), variant: 'link', compact: false, onClick: showHideTable, children: _jsx(Icon, { style: { height: '1.5rem', width: '1.5rem' }, 
                            // icon name is kabab case
                            name: 'list-number', role: 'img', "aria-label": 'numbered list' }) }), _jsx(Link, { href: validUrl, variant: 'link', label: validUrl, icon: true, children: _jsx(Icon, { style: { height: '1.5rem', width: '1.5rem' }, 
                            // icon name is kabab case
