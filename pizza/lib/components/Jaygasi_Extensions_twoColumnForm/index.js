...
function JaygasiExtensionsTwoColumnForm(props) {
    ...
    if (readOnly && readOnly === true || displayMode && displayMode === 'DISPLAY_ONLY') {
        return (_jsx(StyledJaygasiExtensionsTwoColumnFormWrapper, { children: _jsx(FieldGroup, { name: propsToUse.showLabel ? propsToUse.label : '', children: _jsx(Grid, { container: gridContainer, "data-testid": `column-count-${numRegions}`, children: children.map((child, i) => (_jsx(Flex
                    // @ts-ignore
                    , { 
                        // @ts-ignore
                        container: { direction: 'column', alignItems: 'normal', colGap: 1, rowGap: 1.5 }, children: _jsx(DetailsRender, { child: child }) }, `r-${i + 1}`))) }) }) }));
    }
    return (_jsx(StyledJaygasiExtensionsTwoColumnFormWrapper, { children: _jsx(FieldGroup, { name: propsToUse.showLabel ? propsToUse.label : '', children: _jsx(Grid, { container: gridContainer, children: children.map((child, i) => (
                // @ts-ignore
