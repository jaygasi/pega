...
import { CurrencyInput as CosmosCurrency, CurrencyDisplay, NumberDisplay, FieldValueList, Text, useAfterInitialEffect, withConfiguration } from '@pega/cosmos-react-core';
import './create-nonce';
// includes in bundle
import handleEvent from './event-utils';
import { suggestionsHandler } from './suggestions-handler';
import StyledJaygasiExtensionsPizzaCurrencyWrapper from './styles';
...
function JaygasiExtensionsPizzaCurrency(props) {
    const { getPConnect, value = 0, placeholder, validatemessage, label, hideLabel = false, helperText, testId, allowDecimals = true, currencyISOCode = 'USD', alwaysShowISOCode = false, displayMode, additionalProps = {}, variant, formatter = 'defaultCurrency', negative = 'minus-sign', notation = 'standard', isTableFormatter = false, hasSuggestions = false } = props;
    ...
    if (displayMode === 'LABELS_LEFT' ||
        ...
        displayComp = (_jsx(CurrencyDisplay, { value: displayValue, currencyISOCode: currencyISOCode, formattingOptions: {
        ...
        if (['Integer', 'Decimal', 'Percentage', 'Decimal-Auto'].includes(formatter)) {
            ...
            displayComp = (_jsx(NumberDisplay, { value: displayValue, formattingOptions: {
        ...
        switch (displayMode) {
            case 'DISPLAY_ONLY': {
                return (_jsxs(StyledJaygasiExtensionsPizzaCurrencyWrapper, { children: [" ", displayComp, " "] }));
            }
            case 'LABELS_LEFT': {
                return (_jsx(StyledJaygasiExtensionsPizzaCurrencyWrapper, { children: _jsx(FieldValueList, { variant: hideLabel ? 'stacked' : variant, "data-testid": testId, fields: [{ id: '1', name: hideLabel ? '' : label, value: displayComp }] }) }));
            }
            case 'STACKED_LARGE_VAL': {
                return (_jsx(StyledJaygasiExtensionsPizzaCurrencyWrapper, { children: _jsx(FieldValueList, { variant: 'stacked', "data-testid": testId, fields: [
                            {
                                id: '2',
    ...
    return (_jsx(StyledJaygasiExtensionsPizzaCurrencyWrapper, { children: _jsx(CosmosCurrency, { ...additionalProps, label: label, labelHidden: hideLabel, info: validatemessage || helperText, value: currencyValue, status: status, showDecimal: allowDecimals, currencyISOCode: currencyISOCode, alwaysShowISOCode: alwaysShowISOCode, showGroupSeparators: showGroupSeparators, placeholder: placeholder, disabled: disabled, readOnly: readOnly, required: required, "data-testid": testId, onChange: (enteredValue) => {
                if (hasSuggestions) {
                    setStatus(undefined);
                }
