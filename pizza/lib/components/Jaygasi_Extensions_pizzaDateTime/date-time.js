...
function DateFormatter(value, { type = types[1], format = "DD/MM/YYYY", tzone = null, fieldType = '' } = {}) {
    if (!value)
        return value;
    switch (type) {
        case types[1]:
            if (tzone && fieldType !== 'Date')
                return dayjs(value).tz(tzone).format(format); // Date only value should be formatted without timezone
...
export default {
    ...
    convertFromTimezone: (value, tzone) => value && tzone
    ...
    "Date-Time-Default": (value, options) => DateFormatter(value, { ...options, type: "customFormat", format: "lll" }),
    "Time-Default": (value, options) => TimeFormatter(value, {
        ...options,
        type: "customFormat",
        format: "hh:mm A"
    })
