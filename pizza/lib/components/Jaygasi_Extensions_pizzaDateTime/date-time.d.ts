export declare const DateTimeShortFormatter: (value: any, options: any) => any;
declare const _default: {
    "DateTime-Long": (value: any, options: any) => any;
    "DateTime-Short": (value: any, options: any) => any;
    "DateTime-Since": (value: any) => any;
    "Time-Only": (value: any, options: any) => any;
    convertToTimezone: (value: any, options: any) => any;
    convertFromTimezone: (value: any, tzone: any) => any;
    Date: (value: any, options: any) => any;
    "Date-Default": (value: any, options: any) => any;
    "Date-Time-Default": (value: any, options: any) => any;
    "Time-Default": (value: any, options: any) => any;
};
export default _default;
//# sourceMappingURL=date-time.d.ts.map