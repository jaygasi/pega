...
const SimpleInteractivePDFViewer = ({ getPConnect, value, highlightParentProp, searchProperty, height = '600px', debug, }) => {
    ...
    const [resolvedPdfSrc, setResolvedPdfSrc] = useState(value || '');
    useEffect(() => {
        if (value) {
            setResolvedPdfSrc(value);
            return undefined;
        }
        if (!cfgPdfProperty || typeof pConnect?.getValue !== 'function')
            return undefined;
        let cancelled = false;
        let attempts = 0;
        const maxAttempts = 6;
        ...
        const tryResolve = () => {
            ...
            try {
                ...
                if (effectiveDebug)
                    console.debug('SimpleInteractivePDFViewer: error resolving pdf property', cfgPdfProperty, err);
            }
            if (!cancelled && attempts < maxAttempts)
                setTimeout(tryResolve, delayMs);
        };
        tryResolve();
    ...
    const [resolvedParentPage, setResolvedParentPage] = useState(pConnect?.getValue?.(effectiveHighlightParentProp) || {});
    useEffect(() => {
        ...
        const tryResolveParent = () => {
            ...
            try {
                ...
                if (effectiveDebug)
                    console.debug('SimpleInteractivePDFViewer: error resolving parent prop', effectiveHighlightParentProp, err);
            }
            if (!cancelled && attempts < maxAttempts)
                setTimeout(tryResolveParent, delayMs);
        };
        tryResolveParent();
        return () => { cancelled = true; };
    ...
    const pxResultsPages = useMemo(() => {
        ...
        const scanTopLevelArrays = () => {
            ...
            for (const key of Object.keys(obj)) {
                const v = obj[key];
                if (Array.isArray(v) && v.length && v.some((el) => el && (el.pageindex !== undefined || el.pageIndex !== undefined || Array.isArray(el?.pxResults)))) {
                    found.push(...v);
                }
            }
        ...
        const scanNestedArrays = () => {
            ...
            for (const val of Object.values(obj)) {
                if (Array.isArray(val) && val.length && typeof val[0] === 'object') {
                    for (const el of val) {
                        if (el && (el.pageindex !== undefined || el.pageIndex !== undefined || Array.isArray(el?.pxResults)))
                            found.push(el);
                    }
                }
            }
        ...
        try {
            ...
            if (effectiveDebug)
                console.debug('SimpleInteractivePDFViewer: error extracting pxResultsPages', err);
            return [];
        }
    ...
    useEffect(() => {
        if (!resolvedPdfSrc)
            return setFileUrl('');
        if (resolvedPdfSrc.startsWith('http'))
            setFileUrl(resolvedPdfSrc);
        else
            setFileUrl(base64ToBlobUrl(resolvedPdfSrc));
    }, [resolvedPdfSrc]);
    // Find highlights for the current page
    const highlights = pxResultsPages.find((p) => (p.pageindex ?? p.pageIndex ?? 0) === currentPage)?.pxResults || [];
    // Scroll to page when highlight is clicked
    const handleHighlightClick = (pageIdx) => {
    ...
    const domSearch = useCallback((lowercase) => {
        try {
            ...
            prev.forEach((el) => {
                if (el instanceof HTMLElement)
                    el.style.background = '';
                try {
                    el.removeAttribute('data-sipdv-search-highlight');
                }
                catch (e) { /* ignore */ }
            });
            ...
            for (const tl of Array.from(textLayers)) {
                ...
                if (effectiveDebug)
                    console.debug('SimpleInteractivePDFViewer: found in-rendered text, navigated to page', pageIdx);
                return true;
            }
        }
        catch (err) {
            if (effectiveDebug)
                console.debug('SimpleInteractivePDFViewer: DOM search error', err);
        }
        return false;
    }, [effectiveDebug]);
    ...
    const performSearch = useCallback((searchText) => {
        ...
        try {
            for (const p of pxResultsPages) {
                if (pageContains(p, lowercase)) {
                    ...
                    if (typeof pageIdx === 'number' && !Number.isNaN(pageIdx)) {
                        setCurrentPage(pageIdx);
                        if (effectiveDebug)
                            console.debug('SimpleInteractivePDFViewer: found in pxResultsPages, navigated to page', pageIdx);
                        return true;
                    }
                }
            }
            ...
            if (effectiveDebug)
                console.debug('SimpleInteractivePDFViewer: error searching pxResultsPages', err);
        }
    ...
    useEffect(() => {
        try {
            window.simplePdfViewer = window.simplePdfViewer || {};
            window.simplePdfViewer.search = performSearch;
        }
        catch (e) {
            if (effectiveDebug)
                console.debug('SimpleInteractivePDFViewer: failed to expose window API', e);
        }
        const onSearch = (ev) => {
            const detail = ev?.detail || {};
            const searchText = (detail?.searchText || '').toString().trim();
            if (!searchText)
                return;
            performSearch(searchText);
        };
        window.addEventListener('simplePdfViewerSearch', onSearch);
        return () => {
            try {
                if (window.simplePdfViewer)
                    delete window.simplePdfViewer.search;
            }
            catch (e) {
                if (effectiveDebug)
                    console.debug('SimpleInteractivePDFViewer: error removing window API', e);
            }
            window.removeEventListener('simplePdfViewerSearch', onSearch);
        };
    }, [performSearch, effectiveDebug]);
    // Watch the configured Pega clipboard search property and invoke performSearch when it changes
    useEffect(() => {
        if (!effectiveSearchProperty || typeof pConnect?.getValue !== 'function')
            return undefined;
        try {
            ...
            if (effectiveDebug)
                console.debug('SimpleInteractivePDFViewer: error reading search property', effectiveSearchProperty, e);
        }
    ...
    return (_jsxs("div", { style: { position: 'relative', height }, children: [fileUrl && (_jsx(Worker, { workerUrl: workerUrl, children: _jsxs("div", { ref: viewerRootRef, style: { position: 'relative', height: '100%' }, children: [_jsx(Viewer, { fileUrl: fileUrl, plugins: [defaultLayoutPluginInstance], defaultScale: SpecialZoomLevel.PageFit, initialPage: currentPage, onPageChange: ({ currentPage: cp }) => setCurrentPage(cp) }), _jsx("div", { style: { position: 'absolute', left: 0, top: 0, right: 0, bottom: 0, pointerEvents: 'none' }, children: highlights.map((h, i) => {
                                ...
                                return (_jsx("button", { "aria-label": "Highlight", type: "button", style: {
                                        position: 'absolute',
                                        left: `${left}%`,
                                        top: `${top}%`,
                                        width: `${widthVal}%`,
                                        ...
                                        pointerEvents: 'auto',
                                        cursor: 'pointer',
                                    }, onClick: () => handleHighlightClick(currentPage) }, h.id || i));
                            }) })] }) })), !fileUrl && _jsx("div", { style: { padding: 20 }, children: "No PDF source available" })] }));
};
