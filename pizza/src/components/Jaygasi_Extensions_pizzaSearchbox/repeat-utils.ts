...
const updateDateTimeField = (expr, fieldMap, dateOnlyFunctions, timezone) => {
  if (expr.condition) {
    ...
    if (
      fieldMap[expr.condition.lhs.field].type === 'datetime' &&
      ...
      if (expr.condition.rhs?.dateFunction === 'HOURS_OF_DAY') {
        ...
        expr.condition.rhs.value = new Date(
          DateTimeFormatter.convertFromTimezone(expr.condition.rhs.value, timezone)
        ).toISOString();
      }
    }
...
export function getDataViewParameters(parameters, runtimeParams) {
  if (!runtimeParams || !parameters) return {};
...
export function getLabelsForTimeoutModal(tableState) {
  ...
  let {
    heading,
    genericDescription,
    tableNameMessagePrefix,
    tableNameMessageSuffix,
    cancelButtonLabel,
    submitButtonLabel
  } = confirmationModalLabels.TIMEOUT;
...
export function getAssociationFieldsFromQuery(query) {
  ...
  return query?.select.filter(
    ({ field }) => isAssociationField(field) && !isAssociationAlreadyAdded(field, associationNames)
  );
}
...
export function isOpenViewInModal(additionalDetails) {
  return (
    additionalDetails.type === 'OPEN_VIEW' && additionalDetails.mode === 'MODAL' && additionalDetails.params.config.name
  );
}
    ...
    .then((results) =>
      results.forEach((result) => {
        if (result.status === 200 || Array.isArray(result.data)) {
          ...
          if (result.count !== null && result.count !== undefined) count = result.count;
        } else {
