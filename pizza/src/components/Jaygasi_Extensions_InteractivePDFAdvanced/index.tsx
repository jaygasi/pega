...
export interface JaygasiExtensionsInteractivePDFAdvancedProps {
  // Minimal platform hook used by the component; storybook can pass a mock as any.
  getPConnect: () => any;
  PDFReference: string;
  ...
  height?: string;
  // Optional override so host can pass exact worker URL
  workerUrl?: string;
}
...
function processPageContainer(container: any, viewerRootRef: React.RefObject<HTMLDivElement> | null, debug = false) {
  ...
  for (const inner of container.pxResults) {
    ...
    if (e2) {
      if (debug && typeof console !== 'undefined') console.log('[InteractivePDFAdvanced][debug] computed entry (page container):', merged, '=>', e2);
      out.push(e2);
    }
  }
  return out;
...
function computeEntriesFromResults(results: any[], viewerRootRef: React.RefObject<HTMLDivElement> | null, debug = false) {
  ...
  for (const it of results) {
    ...
    if (it && Array.isArray(it.pxResults)) {
      ...
      if (e) {
        if (debug && typeof console !== 'undefined') console.log('[InteractivePDFAdvanced][debug] computed entry (flat):', it, '=>', e);
        entries.push(e);
      }
    }
  }
  if (debug && typeof console !== 'undefined') console.log('[InteractivePDFAdvanced][debug] computeEntriesFromResults final entries:', entries);
  return entries;
}
...
function applyPxResultsToViewer(results: any[], viewerRootRef: React.RefObject<HTMLDivElement> | null, setPixelHighlightsFn: (v: any[]) => void, debug = false): boolean {
  ...
  if (entries.length) {
    if (debug && typeof console !== 'undefined') console.log('[InteractivePDFAdvanced][debug] applyPxResultsToViewer will apply', entries);
    setPixelHighlightsFn(entries);
    if (typeof console !== 'undefined') console.log('[InteractivePDFAdvanced] applyPxResultsToViewer applied', entries);
    return true;
  }
  if (typeof console !== 'undefined') console.warn('[InteractivePDFAdvanced] applyPxResultsToViewer: no entries computed');
  return false;
}
...
function computeEntriesFromGroupedHighlights(
  grouped: Map<number, any[]>,
  viewerRootRef: React.RefObject<HTMLDivElement> | null,
  colorFor: (c?: number) => string
) {
...
export default function JaygasiExtensionsInteractivePDFAdvanced(
  props: Readonly<JaygasiExtensionsInteractivePDFAdvancedProps>
) {
  const {
    getPConnect,
    PDFReference,
  ...
  const additionalPropsCsv: string = config.additionalHighlightProps || '';
  const additionalProps = useMemo(() => additionalPropsCsv.split(',').map((s: string) => s.trim()).filter(Boolean), [additionalPropsCsv]);
  ...
  function safeStringify(v: any) {
    try {
      ...
      if (typeof console !== 'undefined' && typeof console.debug === 'function') console.debug('safeStringify failed', _err);
  ...
  const safeGetValue = useCallback((ref: string) => {
    try {
      return pConnect?.getValue?.(ref);
    } catch (err) {
      if (enableDebugging && typeof console !== 'undefined') console.debug('[InteractivePDFAdvanced] safeGetValue threw for', ref, err);
      return undefined;
    }
  ...
  const traverseDottedPath = useCallback((src: string) => {
  ...
  const tryResolveValue = useCallback((src: string) => {
    ...
    if (typeof traversed !== 'undefined') return traversed;
  ...
  useEffect(() => {
    if (!enableDebugging) return undefined;
    if (typeof pConnect?.getActionsApi !== 'function') return undefined;
    const existing = safeGetValue('.PdfMapping') ?? safeGetValue('PdfMapping');
    if (typeof existing !== 'undefined' && existing != null) {
      if (typeof console !== 'undefined') console.log('[InteractivePDFAdvanced] PdfMapping exists, not overwriting');
      return undefined;
    }
    ...
    const safeParse = (s: string | undefined) => {
      ...
      try {
        ...
        if (enableDebugging && typeof console !== 'undefined' && typeof console.debug === 'function') console.debug('[InteractivePDFAdvanced] safeParse failed', err);
        return null;
      }
    };

    let parsed = safeParse(testPxResultsJson);
    if (!parsed) parsed = { pxResults: [ { id: 't1', confidence: 0.95, area: { left: 0.1, top: 0.1, width: 0.3, height: 0.15, pageindex: 0 } } ] };

    try {
      pConnect.getActionsApi().updateFieldValue('.PdfMapping', parsed);
      if (typeof console !== 'undefined') console.log('[InteractivePDFAdvanced] Wrote sample PdfMapping to clipboard', parsed);
    } catch (err) {
      if (typeof console !== 'undefined') console.warn('[InteractivePDFAdvanced] Failed to write PdfMapping', err);
    }
    return undefined;
  ...
  const fileUrl = useMemo(() => {
    if (!pdfSource) return '';
    if (pdfSource.startsWith('http')) return pdfSource;
  ...
  const resolveConfidence = useCallback(() => {
    if (typeof configuredConfidence === 'number') return configuredConfidence;
  ...
  const processArray = useCallback((raw: any[], allItems: any[], src: string) => {
    ...
    const looksLikePxResults = raw.every((el: any) => el && (el.area || el.id || el.left || el.top));
    if (looksLikePxResults) {
      allItems.push(...raw);
      return;
    }

    if (typeof console !== 'undefined') console.warn('[InteractivePDFAdvanced] Received Array for', src, "but shape not recognized. Expected PageList (array of pages with pxResults) or pxResults array. Value:", raw);
  }, []);

  // Helper to process a single source and add to allItems
  const processSource = useCallback((src: string, allItems: any[]) => {
    if (!src) return;
    const raw = tryResolveValue(src);
    if (typeof console !== 'undefined') {
      console.log('[InteractivePDFAdvanced] highlight source:', src, '->', raw);
      if (typeof console.debug === 'function') console.debug('[InteractivePDFAdvanced] (debug) highlight source:', src, '->', raw);
    }
  ...
  const highlightsByPage = useMemo(() => {
    ...
    allItems.forEach((it) => {
      ...
      if ((it.confidence || 0) < (minConfidence || 0)) return;
      const entry = {
        id: it.id,
  ...
  const highlightColor = (confidence?: number) => {
    const c = confidence ?? 0;
    if (c >= 0.9) return 'rgba(40,167,69,0.6)';
    if (c >= 0.7) return 'rgba(255,193,7,0.6)';
  ...
  const [pixelHighlights, setPixelHighlights] = useState<
    Array<{ id: string; left: number; top: number; width: number; height: number; pageIndex: number; color: string }>
  >([]);
  ...
  const loadTestHighlights = useCallback(() => {
    ...
    try {
      ...
      if (enableDebugging && typeof console !== 'undefined') console.log('[InteractivePDFAdvanced] Loaded test highlights', entries);
    } catch (err) {
      if (typeof console !== 'undefined' && typeof console.debug === 'function') console.debug('loadTestHighlights failed', err);
    }
  }, [enableDebugging]);
  ...
  const handleApplyInput = useCallback((input: any) => {
    if (!input) return;
    let obj: any = input;
    if (typeof obj === 'string') {
      try {
        ...
        if (typeof console !== 'undefined') console.warn('applyPdfHighlights: invalid JSON', err);
        return;
      }
    }
    ...
    else if (obj && Array.isArray(obj.pxResults)) results = obj.pxResults;

    if (!results.length) {
      if (typeof console !== 'undefined') console.warn('[InteractivePDFAdvanced] applyPdfHighlights: no pxResults found');
      return;
    }
    ...
    const tryApplyEntries = () => {
      ...
      if (applied) {
        if (typeof console !== 'undefined') console.log('[InteractivePDFAdvanced] applyPxResultsToViewer applied after', attempts, 'attempt(s)');
        return;
      }

      if (attempts < maxAttempts) {
        if (typeof console !== 'undefined') console.log('[InteractivePDFAdvanced] apply: no entries yet, retrying', attempts);
        setTimeout(tryApplyEntries, delayMs);
        return;
      }

      if (typeof console !== 'undefined') console.warn('[InteractivePDFAdvanced] applyPxResultsToViewer: no entries computed after retries');
    };
  ...
  useEffect(() => {
    ...
    try {
  (window as any).__applyPDFHighlights = apply;
      (window as any).__clearPDFHighlights = () => setPixelHighlights([]);
      msgHandler = (ev: MessageEvent) => {
        try {
          ...
          if (typeof console !== 'undefined' && typeof console.debug === 'function') console.debug('InteractivePDFAdvanced: message handler error', err);
        }
      };
      window.addEventListener('message', msgHandler);
    } catch (err) {
      if (typeof console !== 'undefined' && typeof console.debug === 'function') console.debug('Failed to attach global apply/clear helpers', err);
    }
    return () => {
      try {
        delete (window as any).__applyPDFHighlights;
        delete (window as any).__clearPDFHighlights;
        if (msgHandler) window.removeEventListener('message', msgHandler as any);
      } catch (err) {
        if (typeof console !== 'undefined' && typeof console.debug === 'function') console.debug('Failed to detach global apply/clear helpers', err);
      }
    };
  }, [enableDebugging, handleApplyInput]);

  useEffect(() => {
    const entries: Array<any> = computeEntriesFromGroupedHighlights(highlightsByPage, viewerRootRef, highlightColor);
    ...
    setPixelHighlights((prev) => {
      ...
      for (let i = 0; i < entries.length; i += 1) {
        if (prev[i].id !== entries[i].id) return entries;
  ...
  useEffect(() => {
    if (!onSelectProp) return undefined;
    try {
      ...
      highlightsByPage.forEach((items, pageIndex) => {
        if (items.some((it: any) => String(it.id) === String(sel))) targetPage = pageIndex;
      });

      if (targetPage != null && viewerRootRef?.current) {
        const pageEl = findPageElement(viewerRootRef, targetPage);
        if (!pageEl || typeof pageEl.scrollIntoView !== 'function') return undefined;
        // Use function length as a lightweight probe: older implementations
        // that don't accept options often have length === 0.
        const fnLen = (pageEl.scrollIntoView as any).length || 0;
        if (fnLen > 0) pageEl.scrollIntoView({ behavior: 'smooth', block: 'center' } as ScrollIntoViewOptions);
        else pageEl.scrollIntoView();
      }
    } catch (err) {
      if (enableDebugging && typeof console !== 'undefined' && typeof console.debug === 'function') console.debug('selection->navigate failed', err);
    }
    return undefined;
  ...
  useEffect(() => {
    if (!enableDebugging) return undefined;
    try {
      (window as any).__loadPDFTestHighlights = loadTestHighlights;
    } catch (err) {
      if (enableDebugging && typeof console !== 'undefined' && typeof console.debug === 'function') console.debug('Failed to attach __loadPDFTestHighlights to window', err);
    }
    return () => {
      try {
        delete (window as any).__loadPDFTestHighlights;
      } catch (err) {
        if (enableDebugging && typeof console !== 'undefined' && typeof console.debug === 'function') console.debug('Failed to detach __loadPDFTestHighlights from window', err);
      }
    };
  ...
  const findInRoots = useCallback((propName: string) => {
    if (!propName || typeof propName !== 'string') return undefined;
    const roots = ['pyWorkPage', 'pyCasePage', 'pyCaseInfo', 'pyDefault', 'pzInsPage', 'pxRequestor'];
    // Shallow 2-level search (direct, child, grandchild) with a tiny helper
  const deepFind = (rootObj: any, targetKey: string) => {
      if (rootObj == null || typeof rootObj !== 'object') return undefined;

  const findInObject = (o: any) => (o && typeof o === 'object' && Object.hasOwn(o, targetKey) ? o[targetKey] : undefined);
    ...
    const found = deepFind(parent, propName);
        if (typeof found !== 'undefined') {
          if (typeof console !== 'undefined') console.log('[InteractivePDFAdvanced][debug] found under root:', r, safeStringify(found));
          return { path: `${r}.${propName}`, value: found };
        }
      }
    }
    if (typeof console !== 'undefined') console.log('[InteractivePDFAdvanced][debug] findInRoots: not found for', propName);
    return undefined;
  }, [safeGetValue]);
  ...
  useEffect(() => {
    if (!enableDebugging) return undefined;
    try {
      (window as any).__findPDFProperty = findInRoots;
    } catch (err) {
      if (enableDebugging && typeof console !== 'undefined' && typeof console.debug === 'function') console.debug('Failed to attach __findPDFProperty to window', err);
    }
    return () => {
      try { delete (window as any).__findPDFProperty; } catch (err) { if (enableDebugging && typeof console !== 'undefined' && typeof console.debug === 'function') console.debug('Failed to detach __findPDFProperty', err); }
    };
  ...
  useEffect(() => {
    if (!testPxResultsJson) return undefined;
    let parsed: any;
    try {
      parsed = JSON.parse(testPxResultsJson);
    } catch (err) {
      if (typeof console !== 'undefined') console.warn('[InteractivePDFAdvanced] testPxResultsJson invalid JSON', err);
      return undefined;
    }
    ...
    const tryApply = () => {
      ...
      else if (parsed && Array.isArray(parsed.pxResults)) results = parsed.pxResults;
    ...
    if (!tryApply()) {
      ...
      const tfn = () => {
        ...
        attempts += 1;
        if (tryApply()) return;
        if (attempts < maxAttempts) setTimeout(tfn, delayMs);
      };
  ...
  useEffect(() => {
    ...
    for (const src of sources) {
      try {
        ...
        if (typeof console !== 'undefined') console.log('[InteractivePDFAdvanced][debug] source:', src, '->', safeStringify(val));
      } catch (err) {
        if (typeof console !== 'undefined') console.log('[InteractivePDFAdvanced][debug] getValue threw for', src, err);
      }
    }
    try {
      const grouped: any = {};
      highlightsByPage.forEach((items, pageIndex) => { grouped[pageIndex] = items; });
      if (typeof console !== 'undefined') console.log('[InteractivePDFAdvanced][debug] merged highlightsByPage:', safeStringify(grouped));
    } catch (err) {
      if (typeof console !== 'undefined') console.log('[InteractivePDFAdvanced][debug] failed computing merged highlights', err);
    }
    return undefined;
  ...
  const setPriority = useCallback((el: HTMLElement, prop: string, value: string) => {
    try { el.style.setProperty(prop, value, 'important'); } catch (err) { if (typeof console !== 'undefined' && typeof console.debug === 'function') console.debug('setPriority failed', err); }
  }, []);

  const patchChildAndInner = useCallback((ch: HTMLElement) => {
    ...
    const inner = Array.from(ch.querySelectorAll<HTMLElement>('div, span')).slice(0, 80);
  ...
  const applyPatch = useCallback(() => {
    try {
      const root = viewerRootRef.current as HTMLElement | null;
      if (!root) return;
      ...
      setPriority(toolbar as HTMLElement, 'display', 'flex');
      setPriority(toolbar as HTMLElement, 'flex-direction', 'row');
      setPriority(toolbar as HTMLElement, 'flex-wrap', 'nowrap');
      setPriority(toolbar as HTMLElement, 'align-items', 'center');
      setPriority(toolbar as HTMLElement, 'gap', '8px');
      setPriority(toolbar as HTMLElement, 'white-space', 'nowrap');

      // Position toolbar absolutely across the top of the viewer
      setPriority(toolbar as HTMLElement, 'position', 'absolute');
      setPriority(toolbar as HTMLElement, 'top', '8px');
      setPriority(toolbar as HTMLElement, 'left', '8px');
      setPriority(toolbar as HTMLElement, 'right', '8px');
      setPriority(toolbar as HTMLElement, 'width', 'auto');
      setPriority(toolbar as HTMLElement, 'max-width', 'none');
      setPriority(toolbar as HTMLElement, 'z-index', '2147483647');

      // Apply to buttons
      const buttons = Array.from(toolbar.querySelectorAll<HTMLElement>('button, .rpv-core__toolbar-button, .rpv-core__icon-button, [role="button"]'));
      buttons.forEach((b) => {
      ...
      try {
        const directChildren = Array.from(toolbar.children).slice(0, 60) as HTMLElement[];
        for (const childEl of directChildren) patchChildAndInner(childEl);
      } catch (innerErr) {
        if (typeof console !== 'undefined' && typeof console.debug === 'function') console.debug('InteractivePDFAdvanced: child-level patch failed', innerErr);
      }

      const tbRect = (toolbar as HTMLElement).getBoundingClientRect();
      const currentPaddingTop = parseFloat(getComputedStyle(root).paddingTop || '0') || 0;
      const needed = Math.ceil(tbRect.height + 12);
      if (currentPaddingTop < needed) setPriority(root, 'padding-top', `${needed}px`);
    } catch (e) {
      if (typeof console !== 'undefined' && typeof console.debug === 'function') console.debug('InteractivePDFAdvanced: applyPatch failed', e);
    }
  }, [patchChildAndInner, setPriority]);

  useEffect(() => {
    if (!enablePegaWorkaround) return;
  ...
  return (
    <ViewerWrapper className="jaygasi-advanced-pdf" height={height}>
      {enableSearch && (
      ...
      {fileUrl ? (
        <Worker workerUrl={worker}>
          <div ref={viewerRootRef} style={{ position: 'relative', height: '100%' }}>
            <Viewer fileUrl={fileUrl} plugins={[defaultLayoutPluginInstance]} defaultScale={SpecialZoomLevel.PageFit} />
            {/* absolute overlay container */}
            <div style={{ position: 'absolute', left: 0, top: 0, right: 0, bottom: 0, pointerEvents: 'none' }}>
              {pixelHighlights.map((h) => (
                <Highlight
                  key={h.id}
                  style={{ position: 'absolute', left: `${h.left}px`, top: `${h.top}px`, width: `${h.width}px`, height: `${h.height}px`, pointerEvents: 'auto' }}
                  onClick={() => {
                    if (onSelectProp) pConnect.getActionsApi().updateFieldValue(onSelectProp, h.id);
                  }}
                  $confidenceColor={h.color}
                />
              ))}
            </div>
          </div>
      ...
      {/* Debugging outputs are printed to the console only when enableDebugging=true */}
    </ViewerWrapper>
  );
}
