...
const TableImage = (imageProps) => {
    ...
    imageDisplayComp = (_jsxs(Fragment, { children: [btnVal !== null && (_jsx(Button, { ref: demoButtonRef, variant: 'link', compact: false, onClick: onClick, autoFocus: true, children: btnVal })), images && (_jsx(Lightbox, { items: images, cycle: false, onAfterClose: () => {
                    setImages(null);
                    // @ts-ignore
...
export const urlFormatter = (value, options) => {
    const { displayAs, tableDisplayAs, isTableFormatter, altText, altTextOfImage, propaltTextOfImage, urlLabelSelection, urlLabel, propUrlLabel, widthSel, customWidth } = options;
    let urlFormatComp = null;
    if ((displayAs === 'labelText' || (isTableFormatter && tableDisplayAs === 'link')) &&
        (urlLabel !== '' || propUrlLabel !== '')) {
        urlFormatComp = (_jsx(URLDisplay, { target: '_blank', value: value, displayText: urlLabelSelection === 'constant' ? urlLabel : propUrlLabel, variant: 'link' }));
    }
        ...
        if (isTableFormatter) {
            ...
            urlFormatComp = (_jsx(Image, { src: value, width: width, alt: altText === 'constant' ? altTextOfImage : propaltTextOfImage }));
        }
    }
    else {
        urlFormatComp = _jsx(URLDisplay, { target: '_blank', value: value, displayText: value, variant: 'link' });
    }
    return urlFormatComp;
};
//# sourceMappingURL=text-url.js.map