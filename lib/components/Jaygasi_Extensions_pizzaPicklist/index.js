...
const pushPlaceholderOption = (placeholder, listSourceItems, items, pConnect) => {
    ...
    if (placeholder) {
        items.push(_jsx(Option, { value: '', children: pConnect.getLocalizedValue(placeholder) }, placeholder));
    }
        ...
        // @ts-ignore
        items.push(_jsx(Option, { value: '' }, ''));
    }
};
// Duplicated runtime code from Constellation Design System Component
// props passed in combination of props from property panel (config.json) and run time props from Constellation
// any default values in config.pros should be set in defaultProps at bottom of this file
function JaygasiExtensionsPizzaPicklist(props) {
    const { getPConnect, value, label, hideLabel = false, placeholder, datasource = {}, listType, validatemessage, testId, helperText, additionalProps = {}, displayMode, onRecordChange, variant = 'inline', fieldMetadata = {}, isTableFormatter = false, hasSuggestions = false } = props;
    const { formatter } = props;
    const pConnect = getPConnect();
    const actions = pConnect.getActionsApi();
    const stateProps = pConnect.getStateProps();
    ...
    (listSourceItems || []).forEach((item) => {
        ...
        items.push(
        // @ts-ignore
        _jsx(Option, { value: item.key, children: isDatapage
                ? pConnect.getLocalizedValue(item.text, localePath, pConnect.getLocaleRuleNameFromKeys(localeClass, localeContext, localeName))
                : pConnect.getLocalizedValue(item.value, localePath, pConnect.getLocaleRuleNameFromKeys(localeClass, localeContext, localeName)) }, item.key));
    });
    const isDisplayModeEnabled = displayMode === 'LABELS_LEFT' || displayMode === 'STACKED_LARGE_VAL' || displayMode === 'DISPLAY_ONLY';
    let firstOptionKey = '';
    ...
    if (displayMode === 'LABELS_LEFT' || displayMode === 'DISPLAY_ONLY') {
        ...
        if (selectedLabel === '') {
            displayComp = value || _jsx("span", { "aria-hidden": 'true', children: "\u2013\u2013" });
        }
        return displayMode === 'DISPLAY_ONLY' ? (_jsx(StyledJaygasiExtensionsPizzaPicklistWrapper, { children: displayComp })) : (_jsx(StyledJaygasiExtensionsPizzaPicklistWrapper, { children: _jsx(FieldValueList, { variant: hideLabel ? 'stacked' : variant, "data-testid": testId, fields: [
                    {
                        id: '1',
    ...
    if (displayMode === 'STACKED_LARGE_VAL') {
        ...
        const val = isValDefined ? (_jsx(Text, { variant: 'h1', as: 'span', children: selectedLabel })) : ('');
        return (_jsx(StyledJaygasiExtensionsPizzaPicklistWrapper, { children: _jsx(FieldValueList, { variant: 'stacked', "data-testid": testId, fields: [{ id: '2', name: hideLabel ? '' : label, value: val }] }) }));
    }
    const onResolveSuggestionHandler = (accepted) => {
        suggestionsHandler(accepted, pConnect, setStatus);
    };
    return (_jsx(StyledJaygasiExtensionsPizzaPicklistWrapper, { children: _createElement(Select, { ...additionalProps, label: label, labelHidden: hideLabel, info: validatemessage || helperText, status: status, "data-testid": testId, 
            // @ts-ignore
            key: getPConnect().getRawMetadata().config.value, value: value, disabled: disabled, required: required, readOnly: readOnly, onChange: (event) => {
